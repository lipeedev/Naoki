...
export default class MessageCreateEvent extends Event {
    ...
    async runEvent(message) {
        ...
        const command = this.client.commands.vanilla.filter(cmd => cmd.options.cmdType === 'vanilla').get(cmd) || this.client.commands.vanilla.find(cmd => cmd.options.aliases?.includes(cmd));
        if (!command) return;

        try {
            const cmdD = await this.client.database.commands.findOne({ cmdName: command.name });
            if (!cmdD) {
                await this.client.database.commands.create({ cmdName: command.name });
