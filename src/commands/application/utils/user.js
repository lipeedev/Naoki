      ...
      case 'info': {
        if (user.bot) {
          ...
          const UserApplicationEmbed = new Embed(interaction.user)
            ...
            .setFields({ name: t('commands:user:info:fields:tag'), value: user.tag, inline: true }, { name: t('commands:user:info:fields:id'), value: user.id, inline: true }, { name: t('commands:user:info:fields:created'), value: `<t:${~~(user.createdTimestamp / 1000)}:f> <t:${~~(user.createdTimestamp / 1000)}:R>` });
          ...
          const ApplicationEmbed = new Embed(user)
            ...
            .setFields(
              // @ts-ignore
              { name: t('commands:user:info:bot:fields:key'), value: `\`${Application.verify_key || t('undefineds:bot:verify_key')}\`` }, { name: t('commands:user:info:bot:fields:intents'), value: ApplicationIntents.join('\n'), inline: true },
              // @ts-ignore
              { name: t('commands:user:info:bot:fields:tags'), value: Application?.tags ? Application.tags.join('\n') : t('undefineds:bot:tags'), inline: true }
            );

          if (interaction.guild.members.cache.has(user.id)) UserApplicationEmbed.addFields({ name: t('commands:user:info:fields:joined'), value: `<t:${~~(interaction.guild.members.cache.get(user.id).joinedTimestamp / 1000)}:f> <t:${~~(interaction.guild.members.cache.get(user.id).joinedTimestamp / 1000)}:R>` });
          // @ts-ignore
          ...
          const m = await interaction.reply({
            embeds: !interaction.guild.members.cache.has(user.id) ? [
                        ...
                        new Embed(user)
                            ...
                            .setFields({ name: t('commands:user:info:fields:tag'), value: user.tag, inline: true }, { name: t('commands:user:info:fields:id'), value: user.id, inline: true }, { name: t('commands:user:info:fields:created'), value: `<t:${~~(user.createdTimestamp / 1000)}:f> <t:${~~(user.createdTimestamp / 1000)}:R>` }, { name: t('commands:user:info:fields:joined'), value: `<t:${~~(interaction.guild.members.cache.get(user.id).joinedTimestamp / 1000)}:f> <t:${~~(interaction.guild.members.cache.get(user.id).joinedTimestamp / 1000)}:R>` })
                    ],
